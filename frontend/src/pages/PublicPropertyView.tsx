          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-6 text-sm text-[#6B7C4A] font-light">
              ...
              <a 
                ...
                onClick={(e) => {
                  e.preventDefault()
                  const firstPropertyWithROI = properties.find(p => p.roi_info)
                  if (firstPropertyWithROI) {
                    document.getElementById(`roi-${firstPropertyWithROI.id}`)?.scrollIntoView({ behavior: 'smooth' })
                  }
                }}
        ...
        <nav className="p-6 space-y-4">
          ...
          <a
            ...
            onClick={() => {
              setMenuOpen(false)
              setTimeout(() => {
                // Scroll to first property's ROI section
                const firstPropertyWithROI = properties.find(p => p.roi_info)
                if (firstPropertyWithROI) {
                  document.getElementById(`roi-${firstPropertyWithROI.id}`)?.scrollIntoView({ behavior: 'smooth' })
                }
